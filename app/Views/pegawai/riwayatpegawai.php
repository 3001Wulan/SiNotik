...
pdfBtn.addEventListener('click', function () {
    const doc = new jsPDF({ orientation: 'landscape' }); // Set ke lanskap
    const table = document.querySelector('.data-table');
    const rows = table.querySelectorAll('tr');

    const headers = ['No', 'Tanggal', 'Bidang', 'Judul', 'Notulen', 'Partisipan', 'Isi', 'Dokumentasi'];
    const tableData = [];
    const images = {}; // Objek untuk menyimpan gambar dengan indeks barisnya

    rows.forEach((row, rowIndex) => {
        const cells = row.querySelectorAll('th, td');
        const rowData = Array.from(cells).map(cell => cell.textContent.trim());
        if (rowIndex > 0) { 
            rowData.pop(); // Hapus kolom terakhir yang berisi gambar dari teks tabel
            tableData.push(rowData);

            const imgCell = row.cells[7]; 
            const img = imgCell.querySelector('img');
            if (img) {
                getImageBase64(img).then(base64Img => {
                    images[rowIndex - 1] = base64Img; // Simpan gambar sesuai indeks baris
                }).catch(err => {
    ...
    setTimeout(() => {
        ...
        doc.autoTable({
            head: [headers],
            body: tableData, // Tidak memasukkan teks "Gambar" ke dalam data tabel
            startY: 20,
            margin: { top: 10, bottom: 10, left: 10, right: 10 },
            theme: 'grid',
            columnStyles: {
                6: { cellWidth: 80 }, // Lebar tetap kolom "Isi"
                7: { cellWidth: 35 }, // Lebar tetap kolom "Dokumentasi"
            },
            styles: {
                overflow: 'linebreak',
                fontSize: 10,
    ...
    // Pastikan gambar hanya masuk ke baris data, bukan header
    if (data.row.index > 0 && data.column.index === 7 && images[data.row.index - 1]) {
        const imgWidth = 30;
        const imgHeight = 20;
        const imgX = data.cell.x + (data.cell.width - imgWidth) / 2;
        const imgY = data.cell.y + (data.cell.height - imgHeight) / 2;
        doc.addImage(images[data.row.index - 1], 'PNG', imgX, imgY, imgWidth, imgHeight);
    }
},
    ...
    }, 1000); // Delay untuk memastikan gambar sudah diproses
});
