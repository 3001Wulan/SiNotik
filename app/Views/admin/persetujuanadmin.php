...
window.submitRejection = function() {
    ...
    if (!currentRow) {
        alert('Terjadi kesalahan: Baris yang dipilih tidak ditemukan.');
        console.error('currentRow is null');
        return;
    }

    const meetingCell = currentRow.cells[4];
    if (!meetingCell) {
        alert('Terjadi kesalahan: Data rapat tidak ditemukan.');
        console.error('meetingCell is undefined', currentRow);
        return;
    }

    const meetingId = meetingCell.dataset.meetingId;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            rejectionMessage.textContent = `Rapat "${meetingCell.textContent}" ditolak dengan alasan: ${reason}`;
